...
export default class extends Controller {
  static targets = ["title", "slug", "route", "html", "css", "js", "codeTab", "preview", "previewTitle", "previewUrl", "highlight"]
  ...
  update() {
    if (!this.slugTarget.value) this.slugTarget.value = this.slugify(this.titleTarget.value)
  ...
  selectCode(event) {
    ...
    ;["html", "css", "js"].forEach((name) => { this[name + "Target"].hidden = name !== selected })
    this.highlightCode()
  }

  highlightCode() {
    if (!this.hasHighlightTarget) return
    const active = [this.htmlTarget, this.cssTarget, this.jsTarget].find((input) => !input.hidden) || this.htmlTarget
    let code = active.value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
    code = code
      .replace(/(&lt;\/?)([a-z0-9-]+)/gi, '$1<span class="syntax-tag">$2</span>')
      .replace(/([a-z-]+)=(&quot;|")([^"]*)(&quot;|")/gi, '<span class="syntax-attr">$1</span>=<span class="syntax-value">"$3"</span>')
      .replace(/(&lt;\/?)/g, '<span class="syntax-bracket">$1</span>')
      .replace(/(\/?&gt;)/g, '<span class="syntax-bracket">$1</span>')
    this.highlightTarget.innerHTML = code + "\n"
  }

  syncCodeScroll(event) {
    if (!this.hasHighlightTarget) return
    this.highlightTarget.scrollTop = event.currentTarget.scrollTop
    this.highlightTarget.scrollLeft = event.currentTarget.scrollLeft
  }
  ...
  renderPreview() {
    ...
    const header = "<header class=\"site-header\"><strong>GONCHAROFF.PRO</strong><nav><a href=\"/blog\">Статьи</a><a href=\"/about\">О проекте</a></nav></header>"
